...
Theta(-\frac{dp}{dT}) = re^{-rT}\max(S_T-K,0) - \mathbb{1}_{\{S_T\geq k\}}\frac{S_T}{2T}(ln(\frac{S_T}{S_0})+(r-\delta-\frac{1}{2}\sigma^2)T) \notag\\
$$
...
Pathwise_A <- function(n, S0, r, sigma, T, K, delta, input,s)
{
  ...
  if(input == 'delta')
  {
    ...
    for(i in 1:n)
    {
      ...
      derivatives[i] = exp(-r*T)*(K/S0)^2*s*g(stock[(s+1)-1], 
